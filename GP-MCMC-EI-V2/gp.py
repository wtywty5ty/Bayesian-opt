...
def bayesian_optimisation(slice_sample_num, coor_sigma, burn_in, input_dimension,
                          ...
                          greater_is_better=False, mode = 'OPT', acqui_mode = 'MCMC', acqui_sample_num = 3):
    ...
    for n in range(n_iters):
        ...
        if mode == 'OPT':
            ...
            samples = slice_sampler.sample(init = initial_theta, gp = model)

            # we must make sure the sample data > 0, since hyperparameter must > 0
            theta_holder = [list() for i in range(input_dimension + 2)]
            for idx in range(slice_sample_num):
                if np.all(samples[:,idx]>0):
                    for num_idx in range(input_dimension + 2):
                        theta_holder[num_idx].append(samples[:,idx][num_idx])

            if len(theta_holder[0]) != 0:
                theta_opt = [np.mean(samples_k) for samples_k in theta_holder]
            else:
                # if no positive samples contained in all samples, we just use the abs mean
                theta_opt = np.abs([np.mean(samples_k) for samples_k in samples])

                
            model.set_params(**{"kernel__k1__noise_level": np.abs(theta_opt[0]),
            ...
            if acqui_mode == 'OPT':
                ...
                while(len(sample_theta_list) < acqui_sample_num): # all samples of theta must be valid
                    ...
                    if np.all(one_sample[:,0]>0):
                        ...
                        continue
                '''
                next_sample = integrate_sample(integrate_EI,sample_theta_list, yp, mode, greater_is_better=greater_is_better, bounds=bounds, n_restarts=acqui_eva_num)
            else:

...
def integrate_EI(x, sample_theta_list, evaluated_loss, mode, greater_is_better=False, n_params=1):
    ...
    for theta_set in sample_theta_list:
        ...
        x_to_predict = x.reshape(-1, n_params) #([-1,1])
    ...
    for n in range(n_iters):
        ...
        if random_search:
            ...
            if acqui_mode == 'OPT':
                ...
                sample_num = one_theta.shape[1]
                print("#############################################################")
                #print(sample_num)
                for index in range(sample_num):
                    sample_theta_list.append(one_theta[:,index])
                assert len(sample_theta_list) == acqui_sample_num
                #print(len(sample_theta_list))
                #print(len(sample_theta_list[0]))
